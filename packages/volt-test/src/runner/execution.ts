...
export async function runSuites(config: VoltTestConfig, options: RunSuitesOptions): Promise<void> {
  ...
  const runArtifactRoot = createRunArtifactRoot(
    repoRoot,
    options.artifactDir ?? config.artifactDir,
  );
  ...
  for (const suite of selectedSuites) {
    ...
    writeJsonArtifact(
      join(runArtifactRoot, sanitizePathSegment(suite.name), 'suite-summary.json'),
      {
        ...suiteSummary,
...
async function runSuiteWithRetries(args: {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    const prefixedLogger = withPrefix(
      logger,
      `[volt:test] [${suite.name}] [attempt ${attempt}/${maxAttempts}]`,
    );
    ...
    const captureScreenshot = async (
      name = 'screenshot',
    ): Promise<VoltTestArtifactCaptureResult> => {
