...
mod tests {
    ...
    #[test]
    fn uses_volt_log_then_rust_log_then_default() {
        let from_volt = resolve_log_filter("warn", |key| match key {
            "VOLT_LOG" => Some("debug".to_string()),
            "RUST_LOG" => Some("error".to_string()),
            _ => None,
        });
        assert_eq!(from_volt, "debug");

        let from_rust = resolve_log_filter("warn", |key| match key {
            "RUST_LOG" => Some("info".to_string()),
            _ => None,
        });
        assert_eq!(from_rust, "info");

        let from_default = resolve_log_filter("warn", |_| None);
        assert_eq!(from_default, "warn");
    }
}
