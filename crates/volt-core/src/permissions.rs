...
mod tests {
    ...
    fn test_permission_from_str_all_invalid() {
        ...
        assert!(Permission::from_str_name("Clipboard").is_none()); // case-sensitive
        assert!(Permission::from_str_name("CLIPBOARD").is_none());
        assert!(Permission::from_str_name("filesystem").is_none()); // must be "fs"
        assert!(Permission::from_str_name("global-shortcut").is_none()); // must be "globalShortcut"
    }
    ...
    fn test_permission_equality_and_hash() {
        ...
        set.insert(Permission::Clipboard); // duplicate
        assert_eq!(set.len(), 1);
